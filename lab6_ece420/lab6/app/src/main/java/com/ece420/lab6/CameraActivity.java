...
public class CameraActivity extends Activity implements SurfaceHolder.Callback{
    ...
    public byte[] histEq(byte[] data, int width, int height){
        ...
        for (int i = 0; i < 256; i++) {
            if (cdf[i] == 0) continue;
            int val = Math.round(((cdf[i] - cdfMin) / (float)(total - cdfMin)) * 255.0f);
            if (val < 0) val = 0;
            if (val > 255) val = 255;
            cdf[i] = val;
        }

        // Map original pixels through normalized CDF
        for (int i = 0; i < size; i++) {
            int oldVal = data[i] & 0xFF;
            histeqData[i] = (byte)(cdf[oldVal] & 0xFF);
        }
            ...
            for (int col = 0; col < width; col++) {
                ...
                for (int kr = 0; kr < kSize; kr++) {
                    for (int kc = 0; kc < kSize; kc++) {
                        int rr = row + kr - kCenter;
                        int cc = col + kc - kCenter;

                        // Zero-padding outside image boundaries
                        if (rr < 0 || rr >= height || cc < 0 || cc >= width) continue;

                        int pixelVal = data[rr * width + cc] & 0xFF;
                        sum += kernel[kr][kc] * pixelVal;
                    }
                }

                int convVal = (int)Math.round(sum);
                if (convVal < 0) convVal = 0;
                if (convVal > 255) convVal = 255;
                convData[row * width + col] = convVal;
            }
        }
