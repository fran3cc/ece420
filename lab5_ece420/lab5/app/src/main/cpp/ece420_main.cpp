...
bool lab5PitchShift(float *bufferIn) {
    ...
    if (periodLen > 0) {
        ...
        // Desired new period based on target frequency
        float newPeriod = ((float) F_S) / (float) FREQ_NEW;

        // Ensure we have enough epoch marks to compute local periods
        if (epochLocations.size() >= 3) {
            // Synthesize windowed segments centered at epochs, placed at new epoch positions
            // within the current "present" frame region of bufferOut
            while (newEpochIdx < 2 * FRAME_SIZE) {
                // Find the closest epoch in the input buffer to our current output pointer
                int closestEpochIdx = findClosestInVector(epochLocations, (float) newEpochIdx, 1,
                                                          (int) epochLocations.size() - 1);

                // Compute local period using surrounding epochs
                int p = (epochLocations[closestEpochIdx + 1] - epochLocations[closestEpochIdx - 1]) / 2;
                if (p <= 0) {
                    // Safety: fallback to detected buffer period if local period invalid
                    p = periodLen;
                }

                int winLen = 2 * p;
                int srcStart = epochLocations[closestEpochIdx] - p;
                int dstStart = newEpochIdx - p;

                // Create windowed segment around the epoch
                std::vector<float> segment(winLen, 0.0f);
                for (int i = 0; i < winLen; i++) {
                    int srcIdx = srcStart + i;
                    // Bounds check for srcIdx relative to bufferIn
                    float sample = 0.0f;
                    if (srcIdx >= 0 && srcIdx < BUFFER_SIZE) {
                        sample = bufferIn[srcIdx];
                    }
                    float w = getHanningCoef(winLen, i);
                    segment[i] = sample * w;
                }

                // Overlap-add the windowed segment into the output buffer
                overlapAddArray(bufferOut, segment.data(), dstStart, winLen);

                // Advance to the next synthesized epoch position
                newEpochIdx += (int) newPeriod;
            }
        }
